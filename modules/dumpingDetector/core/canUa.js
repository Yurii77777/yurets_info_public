...
const productsCanUa = require("../data/productsCanUa");

const canUa = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsCanUa;
    let data = [];
    ...
    const scrapeCurrentPage = async (
        ...
        try {
            ...
            try {
                const isInStock = await page.$eval(".btn-cart2 > a", (text) =>
