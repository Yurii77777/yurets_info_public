...
const productsBezpekaSystems = require("../data/productsBezpekaSystems");

const bezpekaSystems = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsBezpekaSystems;
    let data = [];
        ...
        try {
            ...
            try {
                dataObj["currentPrice"] = await page.$eval(
                    ".price > .electro-price > .woocommerce-Price-amount > bdi",
            ...
            try {
                const isInStock = await page.$eval(".stock > span", (text) =>
                    text.textContent.trim()
                );

                isInStock === "Є у наявності"
