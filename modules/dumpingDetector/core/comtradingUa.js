...
const productsComtradingUa = require("../data/productsComtradingUa");

const comtradingUa = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsComtradingUa;
    let data = [];
    ...
    const scrapeCurrentPage = async (
        ...
        try {
            ...
            await page.waitForSelector(".section-product");
            ...
            try {
                dataObj["currentPrice"] = await page.evaluate(() => {
                    const priceNodeChildren = Array.from(
                        document.querySelectorAll(".product-price")[0]
                            .childNodes
                    );
                    return priceNodeChildren[1].textContent.replace(
                        /(\D)/gm,
                        ""
                    );
                });
            ...
            try {
                const isInStock = await page.$eval(".stock_status", (text) =>
