...
const productsFixerComUa = require("../data/productsFixerComUa");

const fixerComUa = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsFixerComUa;
    let data = [];
    ...
    const scrapeCurrentPage = async (
        ...
        try {
            ...
            try {
                const isInStock = await page.$eval(".availability", (text) =>
                    text.textContent.trim()
                );

                isInStock === "Є в наявності"
