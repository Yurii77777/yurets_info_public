...
const productsRozetka = require("../data/productsRozetka");

const rozetka = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsRozetka;
    let data = [];
        ...
        try {
            ...
            try {
                dataObj["currentPrice"] = await page.$eval(
                    ".product-prices__inner > .product-prices__big",
            ...
            try {
                const isInStock = await page.$eval(
                    ".product-statuses > .product-statuses__item > .ng-star-inserted > .status-label",
                    (text) => text.textContent.trim()
                );

                isInStock === "Є в наявності" || isInStock === "Есть в наличии"
