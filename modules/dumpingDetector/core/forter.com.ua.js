...
const productsForterComUa = require("../data/productsForterComUa");

const forterComUa = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsForterComUa;
    let data = [];
        ...
        try {
            ...
            try {
                const isInStock = await page.$eval(
                    ".stock-status-wrap > span",
                    (text) => text.textContent.trim()
                );

                isInStock === "Товар в наличии"
