...
const productsDS = require("../data/productsDS");

const DS = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsDS;
    let data = [];
    ...
    const scrapeCurrentPage = async (
        ...
        try {
            ...
            try {
                const price = await page.$eval(".price.uah", (text) =>
                    text.textContent.replace(/(\D)/gm, "")
                );
                let correctPrice = 0;

                if (!!price.length) {
                    const length = price.length - 2;
                    correctPrice = price.slice(0, length);
                }

                dataObj["currentPrice"] = correctPrice;
            } catch (error) {
                dataObj["currentPrice"] = 0;
            }

            try {
                const isInStock = await page.$eval(".in_stock", (text) =>
