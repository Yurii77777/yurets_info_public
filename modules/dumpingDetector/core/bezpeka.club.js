...
const productsBezpekaClub = require("../data/productsBezpekaClub");

const bezpekaClub = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsBezpekaClub;
    let data = [];
        ...
        try {
            ...
            try {
                dataObj["currentPrice"] = await page.$eval(
                    ".price.tac.single-product-page > .big",
            ...
            try {
                const isInStock = await page.$eval(
                    ".single-product--prod-meta > .meta-icons.prod-meta--item > .meta-icons--item.in-stock-sign.with-txt",
                    (text) => text.textContent
                );

                isInStock === "Есть в наличии"
