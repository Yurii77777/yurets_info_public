...
const productsRekordmarketComUa = require("../data/productsRekordmarketComUa");

const rekordmarketComUa = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsRekordmarketComUa;
    let data = [];
    ...
    const scrapeCurrentPage = async (
        ...
        try {
            ...
            try {
                const isInStock = await page.$eval(".stock", (text) =>
                    text.textContent.replace("Наличие:", "").trim()
                );
...
module.exports = rekordmarketComUa;
