...
const productsbezpekaShopCom = require("../data/productsbezpekaShopCom");

const bezpekaShopCom = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsbezpekaShopCom;
    let data = [];
        ...
        try {
            ...
            try {
                dataObj["currentPrice"] = await page.$eval(
                    ".kartochka_akses-item-buy-price > .price-new",
