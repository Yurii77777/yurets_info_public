...
const productsElmirUa = require("../data/productsElmirUa");

const elmirUa = async (options) => {
    const { browser, sellerName, currentFullDate } = options;
    const { sellerProducts, sellerUrl } = productsElmirUa;
    let data = [];
        ...
        try {
            ...
            try {
                const isInStock = await page.$eval(
                    ".pstatus > .ti-v.ti-st > .ti-t",
                    (text) => text.textContent.trim()
                );

                isInStock === "під замовлення (1‑5 днів)"
